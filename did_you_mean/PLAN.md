...
1. [ ] **Write Failing Hook Invocation Test**  
   - [ ] 1.1. Create test directory if missing (`mkdir -p tests`). **Depends on:** _none_  
   - [ ] 1.2. Create Bats test file `tests/hook_detection.bats`. **Depends on:** 1.1  
   - [ ] 1.3. Add test header and boilerplate to `hook_detection.bats`. **Depends on:** 1.2  
...
3. [ ] **Implement Minimal Hook Stub**  
   - [ ] 3.1. Create `src/hook.sh` defining `command_not_found_handle()` that writes `"DM_HOOK"` into `/tmp/dm_hook_called`. **Depends on:** 1.4  
...
4. [ ] **Refactor Hook Setup**  
   - [ ] 4.1. Ensure all hook logic lives in `src/hook.sh`. **Depends on:** 3.1  
...
5. [ ] **Trim Whitespace Handling**  
   - [ ] 5.1. Write test for SAMPLE_WS expecting no leading/trailing spaces. **Depends on:** 2.2, 4.4  
